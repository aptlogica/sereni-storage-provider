...
func TestNewMinioStorageProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := NewMinioStorageProvider(&tt.cfg)
...
func TestMinioStorageProvider_GetURL(t *testing.T) {
	tests := []struct {
		...
		{
			name: "presigned success",
			presignedGetObjectFunc: func(ctx context.Context, bucket, object string, expiry time.Duration, reqParams url.Values) (*url.URL, error) {
				return &url.URL{Scheme: "https", Host: "example.com", Path: "/path"}, nil
			},
			endpointURLFunc: func() *url.URL {
		...
		{
			name: "presigned error fallback",
			presignedGetObjectFunc: func(ctx context.Context, bucket, object string, expiry time.Duration, reqParams url.Values) (*url.URL, error) {
				return nil, errors.New("presigned error")
			},
			endpointURLFunc: func() *url.URL {
				return &url.URL{Scheme: "https", Host: "minio.example.com"}
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mock := &mockMinioClient{
				presignedGetObjectFunc: tt.presignedGetObjectFunc,
...
func TestMinioStorageProvider_Upload(t *testing.T) {
	tests := []struct {
		name        string
		putObjectFunc func(ctx context.Context, bucket, object string, reader io.Reader, size int64, opts minio.PutObjectOptions) (minio.UploadInfo, error)
		presignedFunc func(ctx context.Context, bucket, object string, expiry time.Duration, reqParams url.Values) (*url.URL, error)
		expectError bool
		expectedURL string
	}{
		{
		...
		},
		{
			...
			presignedFunc: func(ctx context.Context, bucket, object string, expiry time.Duration, reqParams url.Values) (*url.URL, error) {
				url, _ := url.Parse("https://minio.example.com/test-bucket/test.txt?signature=abc")
				return url, nil
			},
			expectError: false,
			expectedURL: "https://minio.example.com/test-bucket/test.txt?signature=abc",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mock := &mockMinioClient{
				...
				presignedGetObjectFunc: tt.presignedFunc,
			}
			provider := &MinioStorageProvider{
				client: mock,
				bucket: "test-bucket",
